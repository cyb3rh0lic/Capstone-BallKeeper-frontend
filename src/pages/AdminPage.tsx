...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<Tab>('PENDING'); // 탭 상태
  const [reservations, setReservations] = useState<Reservation[]>([]); // 표시될 목록
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [rejectReason, setRejectReason] = useState('');

  // 탭에 따라 다른 API를 호출하는 함수
  const fetchReservations = useCallback(async () => {
    setIsLoading(true);
    let endpoint = '/api/admin/reservations/pending'; // 기본값
    ...
    try {
      ...
      setError(`[${activeTab}] 목록 로딩 실패: ${err.message}. (백엔드 API 구현이 필요할 수 있습니다)`);
      setReservations([]); // 오류 발생 시 목록 비우기
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    fetchReservations();
  }, [fetchReservations]); // fetchReservations (즉, activeTab)이 바뀔 때 실행
